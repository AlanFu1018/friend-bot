...
| 層 | 載體 | 內容 | 時效 |
| :--- | :--- | :--- | :--- |
| **第 1 層** 短期對話 | `messages` | 該頻道最近 N 則（`short_term_history_limit`，目前 30） | 數分鐘 |
...
滿足其一即提煉：靜默滿 `listen_debounce_seconds`（4 秒），或佇列滿 `listen_max_queue_messages`（目前 30 則）。
...
- **維度 C 不篩相關性**：只要在短期記憶視窗內講過話就算「在場」，與話題是否相關不論。屬產品取捨——「誰在場」對群聊 bot 確實有用，但 C 來源目前享有與被明確提及者完全相同的待遇（同樣配額、同樣態度指令）。
