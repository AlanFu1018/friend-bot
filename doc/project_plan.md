...
| **Phase 8** | **動態好感度與人際進展系統** | • 規劃 4 階 Tier 傲嬌進展階梯、隱密模式（零系統提示，僅 `/kurisu-profile` 顯示）<br>• 支援 `config.yaml` 配置每日好感上限 `daily_gain_limit`<br>• 詳見 [`doc/persona_favorability_plan.md`](file:///C:/ALL%20FILES/Code/friend-bot/doc/persona_favorability_plan.md) | 📋 規劃完成 |
